...
Self‑revision, model‑as‑judge, and critique‑guided editing. Research on self‑critique and model‑guided revision has demonstrated that prompting a model to identify and repair its own errors can improve various NLG axes, such as factuality and coherence \cite{giulianelli2022pragmatic, zhang2024evaluation}. Recent work treats evaluators as agents or promptable judges that can be used at inference time without retraining \cite{liu2024reife, zhang2024evaluation}, and studies have examined how model‑internal signals can approximate crowd preferences when carefully calibrated \cite{nyberg2021estimating}. Unlike single‑critic loops that return a holistic improvement suggestion, DRG splits critique into two modular readers whose outputs are preserved and hashed per example; this yields inspectable failure modes and enables gate rules that act on objective‑specific failures rather than on an opaque composite. In contrast to learned aggregation of multiple evaluators, which can hide which objective motivated a revision, DRG emphasizes traceability of the decision path.

Safety, refusal behavior, and hedging. A growing literature studies how models control refusals and hedged language when faced with safety‑sensitive prompts or uncertain information \cite{cui2023harmlessness, ivgi2024loops}. Work on harmlessness evaluation formalizes multiple axes---factuality, fairness, toxicity---and shows that policy guidance embedded in prompts can shape output behavior in subtle ways \cite{cui2023harmlessness}. In contrast to approaches that bake policy rules into a single constitutional prompt and thereby bias the judge toward refusal \cite{ivgi2024loops, lu2025argus}, DRG isolates compliance judgment in a dedicated reader that can emit constructive critique rather than an immediate refusal token sequence, enabling a revision attempt that may preserve helpfulness for benign use cases. Empirical analyses that document fallback patterns under safety pressure motivate designs that separate signal sources so defensive behavior does not automatically suppress useful content \cite{bubeck2023sparks, ivgi2024loops}.

Multi‑objective evaluation and modular evaluators. The decomposition of multi‑objective optimization into separate signals is a well‑known strategy in optimization theory and in applied ML; it makes tradeoffs explicit and actionable. In NLG, recent benchmarks interrogate whether a single scalar or a panel of specialized readers better captures multidimensional quality \cite{marco2025reader}. Prior ensemble methods often average or learn to aggregate scores, which can obscure per‑objective failure traces; DRG differs by treating readers as instructionable modules with orthogonal rubrics and by applying deterministic gate logic that triggers a revision only when a reader fails or when readers disagree, preserving clarity about what failed and why.

Evaluation methodology, human validation, and reproducibility. Robust evaluation of evaluation‑driven interventions requires careful instrumentation, matched compute budgets, and statistical rigor; recent guidance emphasizes blinded raters, inter‑rater agreement reporting, and per‑example logging to avoid survivorship bias \cite{nyberg2021estimating}. Studies of judge‑scaling and evaluation agent benchmarks show that per‑case logs and paired statistical tests are essential for attributing improvements to the intervention rather than to selection effects \cite{zhang2024evaluation}. DRG adopts these methodological prescriptions by recording prompt hashes, gate decisions, and reader outputs and by preregistering paired per‑example tests with multiple‑comparison correction \cite{liu2024reife}.

Limitations of prior modular approaches and DRG's positioning. Prior modular systems or ensembling approaches aimed at robustness often combined signals in ways that obscure per‑objective behavior; constitutional approaches inject policy directly into critique, which while useful for safety guarantees can bias judgment toward refusal \cite{ivgi2024loops, lu2025argus}. Unlike these approaches, DRG enforces explicit orthogonality in the reader prompt design and preserves critique expressivity, forwarding either sparse tags or freeform action‑oriented feedback to a constrained revision operator. This design choice balances auditability and actionability: by preserving per‑reader outputs, DRG makes it possible to audit which objective triggered a revision and to measure downstream effects on helpfulness, compliance, refusal, and hedging separately, rather than hiding these tradeoffs in a single scalar.
...
Implementation and reproducibility measures. The pipeline is simple to describe and to implement. For a single example, the procedure executes the generator to produce y0, executes Rq and Rc on y0 to obtain (sq, cq) and (sc, cc), computes decision = g(sq, sc; $\tau$q, $\tau$c, gate), and then either returns y0, returns a canonical refusal, or constructs a revision prompt from the chosen critiques and returns the single revised y1 produced by the generator. Because gate logic and the revision construction are deterministic functions in our implementation, per‑example logs can be used to trace why any particular revision was requested and what textual critique led to the final answer.
...
\begin{itemize}
 \item DIRECT (direct answer without post‑hoc reader revision)
 \item MONOL (monolithic safe/helpful joint reader revision gate)
 \item COMPL (compliance‑priority single reader revision gate)
 \item PBLND (prompt‑blind compliance reader dual gate)
 \item ORDSP (orthogonal dual‑reader either‑fail sparse‑tag revision gate)
 \item ORDDT (orthogonal dual‑reader disagreement‑trigger sparse‑tag revision gate)
 \item ORDFR (orthogonal dual‑reader either‑fail freeform‑critique revision gate)
\end{itemize}
...
Revision calls per example & $\leq$1 \\
Seeds executed & 3 (0,1,2) \\
Gate thresholds (pilot) & $\tau$q = 0.7, $\tau$c = 0.8, $\tau$joint = 0.75 \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{charts/fig_ablation_orthogonal_components.png}
\caption{Figure 4: Fig Ablation Orthogonal Variants}
\label{fig:figure_4_fig_ablation_orthogon}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{charts/fig_pairwise_delta_heatmap.png}
\caption{Figure ---: Fig Pairwise Difference Heatmap}
\label{fig:figure_6_fig_pairwise_differen}
\end{figure}
...
Table 1 reports aggregated per‑condition performance (mean $\pm$ std across seeds) on the recorded execution's Primary metric; these numbers are faithfully reproduced from the archived run artifacts and represent the pilot signal used to guide the preregistered replication.

Table 1 --- Experimental results (--- conditions evaluated; AUTO‑GENERATED FROM EXPERIMENT DATA --- DO NOT MODIFY NUMBERS)

\begin{table}[ht]
\centering
\caption{Performance comparison of different methods on Metric (mean $\pm$ std), n}
\label{tab:2}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrr}
\toprule
\textbf{Method} & \textbf{Metric (mean $\pm$ std)} & \textbf{n} \\
\midrule
\texttt{compliance\_priority\_single\_reader\_revision\_gate} & 0.6667 $\pm$ 0.0722 & 3 \\
\texttt{direct\_answer\_without\_posthoc\_reader\_revision} & 0.6667 $\pm$ 0.0722 & 3 \\
\texttt{monolithic\_safe\_helpful\_joint\_reader\_revision\_gate} & \textbf{0.6250} & 3 \\
\texttt{orthogonal\_dual\_reader\_disagreement\_trigger\_sparse\_tag\_revision\_gate} & 0.7083 $\pm$ 0.0722 & 3 \\
\texttt{orthogonal\_dual\_reader\_either\_fail\_freeform\_critique\_revision\_gate} & 0.7500 $\pm$ 0.1250 & 3 \\
\texttt{orthogonal\_dual\_reader\_either\_fail\_sparse\_tag\_revision\_gate} & 0.7083 $\pm$ 0.0722 & 3 \\
\texttt{prompt\_blind\_compliance\_reader\_dual\_gate} & 0.6667 $\pm$ 0.0722 & 3 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Performance comparison of different methods on Seed 0, Seed 1, Seed 2, Mean}
\label{tab:3}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
\textbf{Method} & \textbf{Seed 0} & \textbf{Seed 1} & \textbf{Seed 2} & \textbf{Mean} \\
\midrule
\texttt{compliance\_priority\_single\_reader\_revision\_gate} & 0.6250 & 0.6250 & 0.7500 & 0.6667 \\
\texttt{direct\_answer\_without\_posthoc\_reader\_revision} & 0.6250 & 0.6250 & 0.7500 & 0.6667 \\
\texttt{monolithic\_safe\_helpful\_joint\_reader\_revision\_gate} & 0.6250 & 0.6250 & 0.6250 & 0.6250 \\
\texttt{orthogonal\_dual\_reader\_disagreement\_trigger\_sparse\_tag\_revision\_gate} & 0.7500 & 0.6250 & 0.7500 & 0.7083 \\
\texttt{orthogonal\_dual\_reader\_either\_fail\_freeform\_critique\_revision\_gate} & 0.6250 & 0.7500 & 0.8750 & 0.7500 \\
\texttt{orthogonal\_dual\_reader\_either\_fail\_sparse\_tag\_revision\_gate} & 0.6250 & 0.7500 & 0.7500 & 0.7083 \\
\texttt{prompt\_blind\_compliance\_reader\_dual\_gate} & 0.6250 & 0.6250 & 0.7500 & 0.6667 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Performance comparison of different methods on Mean difference, t statistic, p‑value}
\label{tab:4}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrr}
\toprule
\textbf{Comparison (ORDFR $-$ baseline)} & \textbf{Mean difference} & \textbf{t statistic} & \textbf{p‑value} \\
\midrule
ORDFR vs orthogonal sparse‑tag (ORDSP) & 0.0417 & 1.0 & --- \\
ORDFR vs direct‑answer (DIRECT) & 0.0833 & --- & --- \\
ORDFR vs monolithic joint‑reader (MONOL) & 0.125 & --- & 0.0795 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{itemize}
 \item Framework overview: charts/framework\_diagram.png
 \item Primary metric across conditions: charts/fig\_main\_results.png
 \item Pairwise delta heatmap: charts/fig\_pairwise\_delta\_heatmap.png
\end{itemize}
...
\caption{Comparison of the primary\_metric across instruction conditions}
\label{fig:comparison_of_the_primary_metr}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{charts/fig_pairwise_delta_heatmap.png}
\caption{Pairwise delta heatmap of primary\_metric differences between methods}
\label{fig:pairwise_delta_heatmap_of_prim}
\end{figure}
...
\section{Appendix A: Prompt templates and reproducibility artifacts}

\label{sec:appendix_a_prompt_templates_and_reproduc}
